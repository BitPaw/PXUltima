...
PXSize PXSBPChannelHeaderParse(PXSBPChannelHeader* const sbpChannelHeader, PXFile* const dataStream)
{
	...
	PXMemoryClear(sbpChannelHeader, sizeof(PXSBPChannelHeader));
...
void PXSBPPackageHeaderConstruct(SBPPackageHeader* const sbpPackageHeader)
{
	SBPPackageHeaderSet(sbpPackageHeader, PXInt32Make('#', '#', '#', '#'), SourceInvalid, TargetInvalid, 0, PXNull);
}

void PXSBPPackageHeaderDestruct(SBPPackageHeader* const sbpPackageHeader)
{

}

void PXSBPPackageHeaderSet
(
...
void PXSBPPackageHeaderPrint(SBPPackageHeader* const sbpData)
{
	...
	if(sbpData->SourceID >= (unsigned int)SourceLimitMaximum)
	{
		...
		sprintf_s(sourceText, 10, "%s", textINN);
	}
	else
	{
		sprintf_s(sourceText, 10, "%8X", sbpData->Source);
	}
	...
	if(sbpData->TargetID >= (unsigned int)TargetLimitMaximum)
	{
		...
		sprintf_s(targetText, 10, "%s", textINN);
	}
	else
	{
		sprintf_s(targetText, 10, "%8X", sbpData->Target);
	}
...
PXSize PXSBPPackageParse(SBPPackageHeader* const sbpPackageHeader, PXFile* const dataStream)
{
	...
	PXSBPPackageHeaderConstruct(sbpPackageHeader);
...
PXSize PXSBPPackageHeaderPackageFileSerialize(SBPPackageHeaderPackageFile* const sbpDataPackageFile, void* outputData, const PXSize outputDataSize)
{
	return 0;
}

PXSize PXSBPPackageHeaderPackageIamParse(SBPPackageHeader* const sbpDataPackage, SBPPackageHeaderPackageIam* const sbpDataPackageIam)
{
	PXFile dataStream;
...
PXSize PXSBPPackageHeaderPackageIamSerialize(SBPPackageHeader* const sbpDataPackage, SBPPackageHeaderPackageIam* const sbpDataPackageIam)
{
	PXFile dataStream;
	...
	PXMemoryClear(sbpDataPackageIam, sizeof(SBPPackageHeaderPackageIam));

	// Add name
	PXText userName;
	PXTextConstructNamedBufferA(&userName, nameBuffer, 128);

	const PXBool gotName = PXUserNameGet(&userName);

	PXFileWriteI8U(&dataStream, userName.Format);
	PXFileWriteI16UE(&dataStream, userName.SizeUsed, PXEndianLittle);
	PXFileWriteB(&dataStream, userName.TextA, userName.SizeUsed);

	PXSBPPackageHeaderSet(sbpDataPackage, SBPPackageHeaderPackageIamID, SourceMe, TargetServer, dataStream.DataCursor, PXNull);

...
void PXAPI PXColorPickerRegisterToECS(PXECSRegisterInfo PXREF pxECSRegisterInfo)
{
    pxECSRegisterInfo->InfoStatic = &PXColorPickerInfoStatic;
    pxECSRegisterInfo->InfoDynamic = &PXColorPickerInfoDynamic;
}
...
{
    //PXCamera PXREF pxCamera = PXRendererCameraCurrent(pxDrawInfo->Rendrer);
    PXWindow PXREF pxWindow = pxColorPicker->WindowBase;

    float x = 0;//pxDrawInfo->RectangleXYWH.X;
    float y = 0;//pxDrawInfo->RectangleXYWH.Y;
    ...
    */
    ...
    PXColorHSVToRGBF(&hsvHue, &colHue);
    PXColorHSVToRGBF(&hsvWhite, &colWhite);
    PXColorHSVToRGBF(&hsvBlack, &colBlack);
    colHue.Alpha = 1.0f;
    colWhite.Alpha = 1.0f;
    colBlack.Alpha = 1.0f;
    ...
    selectRectangle[0].X = centerX + innerRadius * PXMathCosinusRADF32(angleRad+ widthAgng);
    selectRectangle[0].Y = centerY + innerRadius * PXMathSinusRADF32(angleRad+ widthAgng);

    selectRectangle[1].X = centerX + outerRadius * PXMathCosinusRADF32(angleRad+ widthAgng);
    selectRectangle[1].Y = centerY + outerRadius * PXMathSinusRADF32(angleRad+ widthAgng);

    selectRectangle[2].X = centerX + outerRadius * PXMathCosinusRADF32(angleRad- widthAgng);
    selectRectangle[2].Y = centerY + outerRadius * PXMathSinusRADF32(angleRad- widthAgng);
   ...
   // RotatePoint(knobPosition.X, knobPosition.Y, angleRad, &knobPosition.X, &knobPosition.Y);
    ...
    // V slider
    {
        ...
        PXColorRGBAF c0, c1;
        PXColorHSVToRGBF(&hsv0, &c0);
        PXColorHSVToRGBF(&hsv1, &c1);
        c0.Alpha = 1.0f;
        c1.Alpha = 1.0f;
    ...
    // Alpha slider with checker background
    {
        ...
        PXColorRGBAF opaque = { currentRGBA.Red, currentRGBA.Green, currentRGBA.Blue };
        PXColorRGBAF transparent = { currentRGBA.Red * 0.0f,
...
PXResult PXAPI PXColorPickerDraw(PXColorPicker PXREF pxColorPicker, PXDrawInfo PXREF pxDrawInfo)
{
    ...
    PXF32 scaling = 0.80f;
    int width = 0;//(pxDrawInfo->RectangleXYWH.Width - pxDrawInfo->RectangleXYWH.X) / 2;
    int height = 0;//(pxDrawInfo->RectangleXYWH.Height - pxDrawInfo->RectangleXYWH.Y) / 2;
    int widthS = 0;//(pxDrawInfo->RectangleXYWH.Width * scaling - pxDrawInfo->RectangleXYWH.X * scaling) / 2;
    int heightS = 0;// (pxDrawInfo->RectangleXYWH.Height * scaling - pxDrawInfo->RectangleXYWH.Y * scaling) / 2;
...
#if OSUnix
#elif OSWindows
    HDC hdc = PXWindowDCGet(pxColorPicker->WindowBase);

...
PXActionResult PXAPI PXMultiMediaDeviceStop(PXAudioMultiMedia* const pxAudioMultiMedia, PXAudioDevice* const pxAudioDevice)
{
    ...
    switch(pxAudioDevice->Type)
    {
        case PXAudioDeviceTypeInput:
        {
            ...
            HWAVEIN waveHandle = (HWAVEIN)pxAudioDevice->Info.Handle.DirectXInterface;
...
PXActionResult PXAPI PXMultiMediaInitialize(PXAudioMultiMedia* const pxAudioMultiMedia, PXAudioInitializeInfo* const pxAudioInitializeInfo)
{
    ...
    // Library
    {
        const PXActionResult loadLibResult = PXLibraryOpenA(&pxAudioMultiMedia->MultiMediaLibrary, PXMultimediaLibraryName);
...
PXActionResult PXAPI PXMultiMediaDevicePitchIncrease(PXAudioMultiMedia* const pxAudioMultiMedia, PXAudioDevice* const pxAudioDevice, PXF32 amount)
{
    //assert(amount > 0.0f);
...
PXActionResult PXAPI PXMultiMediaDevicePitchReduce(PXAudioMultiMedia* const pxAudioMultiMedia, PXAudioDevice* const pxAudioDevice, PXF32 amount)
{
    //assert(amount > 0.0f);

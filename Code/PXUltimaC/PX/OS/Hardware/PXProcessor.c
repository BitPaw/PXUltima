...
PXProcessorBrand PXAPI PXProcessorBrandDetect(const char* name)
{
    ...
    if(PXTextCompareA("AMDisbetter!", bufferSize, name, bufferSize, 0)) return ProcessorBrandAMD;
    if(PXTextCompareA("AuthenticAMD", bufferSize, name, bufferSize, 0)) return ProcessorBrandAMD;
    if(PXTextCompareA("CentaurHauls", bufferSize, name, bufferSize, 0)) return ProcessorBrandWinChip;
    if(PXTextCompareA("CyrixInstead", bufferSize, name, bufferSize, 0)) return ProcessorBrandCyrix;
    if(PXTextCompareA("GenuineIntel", bufferSize, name, bufferSize, 0)) return ProcessorBrandIntel;
    if(PXTextCompareA("TransmetaCPU", bufferSize, name, bufferSize, 0)) return ProcessorBrandTransmeta;
    if(PXTextCompareA("GenuineTMx86", bufferSize, name, bufferSize, 0)) return ProcessorBrandTransmeta;
    if(PXTextCompareA("Geode by NSC", bufferSize, name, bufferSize, 0)) return ProcessorBrandNationalSemiconductor;
    if(PXTextCompareA("NexGenDriven", bufferSize, name, bufferSize, 0)) return ProcessorBrandNexGen;
    if(PXTextCompareA("RiseRiseRise", bufferSize, name, bufferSize, 0)) return ProcessorBrandRise;
    if(PXTextCompareA("SiS SiS SiS ", bufferSize, name, bufferSize, 0)) return ProcessorBrandSiS;
    if(PXTextCompareA("UMC UMC UMC ", bufferSize, name, bufferSize, 0)) return ProcessorBrandUMC;
    if(PXTextCompareA("VIA VIA VIA ", bufferSize, name, bufferSize, 0)) return ProcessorBrandVIA;
    if(PXTextCompareA("Vortex86 SoC", bufferSize, name, bufferSize, 0)) return ProcessorBrandDMAndPVortex;
    if(PXTextCompareA("  Shanghai  ", bufferSize, name, bufferSize, 0)) return ProcessorBrandZhaoxin;
    if(PXTextCompareA("HygonGenuine", bufferSize, name, bufferSize, 0)) return ProcessorBrandHygon;
    if(PXTextCompareA("E2K MACHINE", bufferSize, name, bufferSize, 0)) return ProcessorBrandMCSTElbrus;
    if(PXTextCompareA("MiSTer AO486", bufferSize, name, bufferSize, 0)) return ProcessorBrandao486;
    if(PXTextCompareA("bhyve bhyve ", bufferSize, name, bufferSize, 0)) return ProcessorBrandbhyve;
    if(PXTextCompareA(" KVMKVMKVM  ", bufferSize, name, bufferSize, 0)) return ProcessorBrandKVM;
    if(PXTextCompareA("TCGTCGTCGTCG", bufferSize, name, bufferSize, 0)) return ProcessorBrandQEMU;
    if(PXTextCompareA("Microsoft Hv", bufferSize, name, bufferSize, 0)) return ProcessorBrandMicrosoftHyperVOrVirtualPC;
    if(PXTextCompareA("MicrosoftXTA", bufferSize, name, bufferSize, 0)) return ProcessorBrandMicrosoftx86ToARM;
    if(PXTextCompareA(" lrpepyh  vr", bufferSize, name, bufferSize, 0)) return ProcessorBrandParallels;
    if(PXTextCompareA("VMwareVMware", bufferSize, name, bufferSize, 0)) return ProcessorBrandVMware;
    if(PXTextCompareA("XenVMMXenVMM", bufferSize, name, bufferSize, 0)) return ProcessorBrandXenHVM;
    if(PXTextCompareA("ACRNACRNACRN", bufferSize, name, bufferSize, 0)) return ProcessorBrandProjectACRN;
    if(PXTextCompareA(" QNXQVMBSQG ", bufferSize, name, bufferSize, 0)) return ProcessorBrandQNXHypervisor;
    if(PXTextCompareA("VirtualApple", bufferSize, name, bufferSize, 0)) return ProcessorBrandAppleRosetta;

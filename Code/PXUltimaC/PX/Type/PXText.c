...
PXSize PXAPI PXTextAppend(PXText PXREF currentString, const PXText PXREF appendingString)
{
...
PXSize PXAPI PXTextAppendC(PXText PXREF currentString, const char character)
{
    currentString->A[currentString->SizeUsed++] = character;

    return 1;
}

PXSize PXAPI PXTextAppendA(PXText PXREF currentString, const char PXREF appaendString, const char appaendStringSize)
{
...
PXSize PXAPI PXTextAppendW(wchar_t PXREF dataString, const PXSize dataStringSize, const wchar_t PXREF appaendString, const PXSize appaendStringSize)
{
...
PXSize PXAPI PXTextAppendF(PXText PXREF pxText, const char PXREF format, ...)
{
...
PXSize PXAPI PXTextFindLast(const PXText PXREF stringSource, const PXText PXREF stringTarget, PXText PXREF stringResult)
{
    ...
    if(stringSource->Format == TextFormatASCII && stringTarget->Format == TextFormatASCII) // Is Ascii
    {
        ...
        for(; i > 0 && !found; --i)
        {
            ...
            if(found)
            {
                found = PXMemoryCompare
                (
                    stringTarget->A,
                    stringTarget->SizeUsed, 
                    stringSource->A + i, 
                    i,
                    PXFalse
                );
...
PXResult PXAPI PXTextCreateCopy(PXText PXREF pxText, const PXText PXREF pxTextSource)
{
    *pxText = *pxTextSource;
    ...
    pxText->Data4 = PXMemoryHeapCallocT(PXByte, pxTextSource->SizeUsed+1);

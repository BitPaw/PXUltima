...
//PXGameCard _pxGameCard;

void PXAPI PXSpaceGridRegisterToECS(PXECSRegisterInfo PXREF pxECSRegisterInfo)
{
    pxECSRegisterInfo->InfoStatic = &PXSpaceGridRegisterInfoStatic;
    pxECSRegisterInfo->InfoDynamic = &PXSpaceGridRegisterInfoDynamic;
}
...
{   
    ...
    else
    {
        ...
        //pxSpaceGridCreateInfo->Window.Engine = 
...
PXResult PXAPI PXSpaceGridDraw(PXSpaceGrid PXREF pxSpaceGrid, PXDrawInfo PXREF pxDrawInfo)
{
   ...
   // glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    ...
    PXCameraGLFPP(pxCamera, PXCameraPerspective3D);
    ...
    pxDrawInfo->Behaviour = PXWindowAllignLeft | PXWindowAllignTop;
    //pxDrawInfo->RectangleXYWH.X = 0;
    //pxDrawInfo->RectangleXYWH.Y = 0;


    pxDrawInfo->Color.Red = 0.3;
    pxDrawInfo->Color.Green = 0.3;
    pxDrawInfo->Color.Blue = 0.3;
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);

    PXWindowDrawText(pxWindowBase, pxDrawInfo);


    
    pxDrawInfo->Color.Red = 0.0;
    pxDrawInfo->Color.Green = 0.0;
    pxDrawInfo->Color.Blue = 0.0;
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);

    PXWindowDrawText(pxWindowBase, pxDrawInfo);


    //PXGameTick(&_pxGameCard);

    //PXGameCardDraw(&_pxGameCard, pxDrawInfo);

...
void PXAPI PXBrushRegisterToECS(PXECSRegisterInfo PXREF pxECSRegisterInfo)
{
    pxECSRegisterInfo->InfoStatic = &PXBrushRegisterInfo;
    pxECSRegisterInfo->InfoDynamic = &PXBrushRegisterInfoDynamic;
}
...
PXResult PXAPI PXBrushCreate(PXBrush** pxBrushRef, PXBrushCreateInfo PXREF pxBrushCreateInfo)
{
    ...
    PXECSInfoFlagStateAdd(&pxBrush->Info, PXECSFlagBehaviourisOK);
...
{
    pxWindowBrush->Info.FlagListSettings |= PXWindowBrushBehaviourColorEmbeded;
